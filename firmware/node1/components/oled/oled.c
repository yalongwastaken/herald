...
static esp_err_t oled_draw_pixel(oled_t *oled, uint8_t x, uint8_t y, bool color) {
    ...
    if (color) {
        oled->buffer[index] |= (1 << bit);   // set pixel
    }
    else {
        oled->buffer[index] &= ~(1 << bit);  // clear pixel
    }
...
static esp_err_t oled_draw_char(oled_t *oled, uint8_t x, uint8_t y, char c, const oled_font_t *font) {
    ...
    for (uint8_t col = 0; col < font->width; col++) {
        ...
        for (uint8_t row = 0; row < font->height; row++) {
            ...
            // bounds check (prevents overflow near edges)
...
esp_err_t oled_init(i2c_master_dev_handle_t dev, oled_t *oled) {
    ...
    // display OFF
    ret = oled_send_cmd(oled, OLED_CMD_DISPLAY_OFF);
    if (ret != ESP_OK) return ret;

    // memory addressing mode
    ret = oled_send_cmd(oled, OLED_CMD_SET_MEM_ADDR_MODE);
    if (ret != ESP_OK) return ret;

    ret = oled_send_cmd(oled, OLED_ADDR_MODE_HORIZONTAL);
    if (ret != ESP_OK) return ret;

    // page addressing
    ret = oled_send_cmd(oled, OLED_CMD_SET_PAGE_START_ADDR);
    if (ret != ESP_OK) return ret;

    // scan direction
    ret = oled_send_cmd(oled, OLED_CMD_SET_COM_SCAN_DIR);
    if (ret != ESP_OK) return ret;

    // column setup
    ret = oled_send_cmd(oled, OLED_CMD_SET_LOW_COLUMN);
    if (ret != ESP_OK) return ret;

    ret = oled_send_cmd(oled, OLED_CMD_SET_HIGH_COLUMN);
    if (ret != ESP_OK) return ret;

    // start line
    ret = oled_send_cmd(oled, OLED_CMD_SET_START_LINE);
    if (ret != ESP_OK) return ret;

    // contrast
    ret = oled_send_cmd(oled, OLED_CMD_SET_CONTRAST);
    if (ret != ESP_OK) return ret;

    ret = oled_send_cmd(oled, 0xFF);
    if (ret != ESP_OK) return ret;

    // segment remap
    ret = oled_send_cmd(oled, OLED_CMD_SEG_REMAP);
    if (ret != ESP_OK) return ret;

    // display mode
    ret = oled_send_cmd(oled, OLED_CMD_NORMAL_DISPLAY);
    if (ret != ESP_OK) return ret;

    // multiplex ratio
    ret = oled_send_cmd(oled, OLED_CMD_SET_MUX_RATIO);
    if (ret != ESP_OK) return ret;

    ret = oled_send_cmd(oled, OLED_MUX_64);
    if (ret != ESP_OK) return ret;

    // RAM behavior
    ret = oled_send_cmd(oled, OLED_CMD_DISPLAY_FOLLOW_RAM);
    if (ret != ESP_OK) return ret;

    // offset
    ret = oled_send_cmd(oled, OLED_CMD_SET_DISPLAY_OFFSET);
    if (ret != ESP_OK) return ret;

    ret = oled_send_cmd(oled, OLED_OFFSET_NONE);
    if (ret != ESP_OK) return ret;

    // clock
    ret = oled_send_cmd(oled, OLED_CMD_SET_CLOCK_DIV);
    if (ret != ESP_OK) return ret;

    ret = oled_send_cmd(oled, OLED_CLOCK_DIV_DEFAULT);
    if (ret != ESP_OK) return ret;

    // precharge
    ret = oled_send_cmd(oled, OLED_CMD_SET_PRECHARGE);
    if (ret != ESP_OK) return ret;

    ret = oled_send_cmd(oled, OLED_PRECHARGE_DEFAULT);
    if (ret != ESP_OK) return ret;

    // COM pins
    ret = oled_send_cmd(oled, OLED_CMD_SET_COM_PINS);
    if (ret != ESP_OK) return ret;

    ret = oled_send_cmd(oled, OLED_COM_PINS_CONFIG);
    if (ret != ESP_OK) return ret;

    // VCOM detect
    ret = oled_send_cmd(oled, OLED_CMD_SET_VCOM_DETECT);
    if (ret != ESP_OK) return ret;

    ret = oled_send_cmd(oled, OLED_VCOM_DEFAULT);
    if (ret != ESP_OK) return ret;

    // charge pump
    ret = oled_send_cmd(oled, OLED_CMD_CHARGE_PUMP);
    if (ret != ESP_OK) return ret;

    ret = oled_send_cmd(oled, OLED_CHARGE_PUMP_ENABLE);
    if (ret != ESP_OK) return ret;

    // display ON
    ret = oled_send_cmd(oled, OLED_CMD_DISPLAY_ON);
    if (ret != ESP_OK) return ret;
...
esp_err_t oled_flush(oled_t *oled) {
    ...
    // flush oled buffer
    return oled_send_data(oled, oled->buffer, sizeof(oled->buffer));
}
...
esp_err_t oled_write_string(oled_t *oled, uint8_t x, uint8_t y, const char *str, const oled_font_t *font) {
    ...
    for (size_t i = 0; str[i] != '\0'; i++) {
        ...
        // draw character
        ret = oled_draw_char(oled, cursor_x, y, c, font);
        if (ret != ESP_OK) return ret;

        // advance cursor
        cursor_x += font->width + 1;

        // stop if we exceed screen width
